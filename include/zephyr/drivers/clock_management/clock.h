...
/**
 * @brief Name for parent clock array
 */
#define Z_CLOCK_PARENT_ARRAY_NAME(node_id) \
	_CONCAT(__clock_parents_, Z_CLOCK_DT_CLK_ID(node_id))
...
#define ROOT_CLOCK_DT_DEFINE(node_id, hw_data, api, ...)                       \
	const struct clk *const Z_CLOCK_PARENT_ARRAY_NAME(node_id)[] = {NULL}  \
	Z_CLOCK_BASE_DEFINE(node_id, Z_CLOCK_DT_CLK_ID(node_id), hw_data,      \
			    api, Z_CLOCK_PARENT_ARRAY_NAME(node_id),           \
